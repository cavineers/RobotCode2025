...
public class ModuleIOSim implements ModuleIO {
    ...
    driveSim =
        new DCMotorSim(
            LinearSystemId.createDCMotorSystem(
                driveGearbox, 0.025, 1 / ModuleConstants.kDriveMotorGearRatio),
            driveGearbox);
    turnSim =
        new DCMotorSim(
            LinearSystemId.createDCMotorSystem(
                turnGearbox, 0.025, 1 / ModuleConstants.kTurningMotorGearRatio),
            turnGearbox);
  ...
  public void updateInputs(ModuleIOInputs inputs) {
    ...
    if (turnClosedLoop) {
      turnAppliedVolts =
          turnController.calculate(turnSim.getAngularPositionRad() % (2 * Math.PI) - Math.PI);
    } else {
    ...
    inputs.turnPosition =
        new Rotation2d(
            turnSim.getAngularPositionRad() % (2 * Math.PI)
                - Math.PI); // this may cause issues with odometry Will see IRL
